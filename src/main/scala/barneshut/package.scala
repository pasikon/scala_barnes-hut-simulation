...
package object barneshut {
  ...
  extends Quad {
    val (mass: Float, massX: Float, massY: Float) = (
      ...
      bodies.foldLeft(0f)((acc, bod) => acc + bod.mass * bod.x) / bodies.foldLeft(0f)(_ + _.mass) : Float,
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Leaf(_, _, _, bodies) =>
          ...
          bodies.foreach(bo => addForce(bo.mass, bo.x, bo.y))
        case fo: Fork =>
          // see if node is far enough from the body,
          // or recursion is needed
          val dist = distance(this.x, this.y, fo.centerX, fo.centerY)
          if (quad.size / dist < theta) {
            ...
            addForce(fo.mass, fo.massX, fo.massY)
          }
      }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      //TODO: out of boundaries
      val sstx = (boundaries.maxX - boundaries.minX) / SECTOR_PRECISION
      val ssty = (boundaries.maxY - boundaries.minY) / SECTOR_PRECISION
      val xidx: Int = ((b.x - boundaries.minX) / sstx).toInt
      val yidx: Int = ((b.y - boundaries.minY) / ssty).toInt
      this(xidx, yidx) += b
      this
    }
